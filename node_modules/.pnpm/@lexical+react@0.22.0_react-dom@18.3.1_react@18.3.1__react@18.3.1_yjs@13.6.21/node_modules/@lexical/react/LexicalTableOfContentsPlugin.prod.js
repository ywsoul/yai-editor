...
exports.TableOfContentsPlugin=function({children:e}){let [t,n]=x.useState([]),[f]=h.useLexicalComposerContext();x.useEffect(()=>{let c=[];f.getEditorState().read(()=>{let k=b=>{for(const d of b.getChildren())q.$isHeadingNode(d)?c.push([d.getKey(),d.getTextContent(),d.getTag()]):w.$isElementNode(d)&&k(d)};k(w.$getRoot());n(c)});let r=f.registerUpdateListener(({editorState:k,dirtyElements:b})=>{k.read(()=>{const d=a=>{for(const g of a.getChildren())q.$isHeadingNode(g)?(a=B(g),c=z(a,g,c),n(c)):w.$isElementNode(g)&&
